...
* **Top-P Sampling & Min-Heap Candidate Filtering (`src/sampler.zig`):** Zero-copy host-visible GPU logits sampling with standard $30.0$ tanh softcapping (`ref/modeling_gemma4.py`), temperature scaling ($0.7$), top-p filtering ($0.95$), localized repetition suppression ($1.05$), and a fast min-heap tracking the true Top-64 logit candidates across the full 262k vocabulary (eliminating first-come-first-served index truncation).
* **Inter-Layer Fused Normalization & Barrier Elimination (`src/gpu/model_gpu.zig`, `src/gpu/model_dispatch.zig`):** Chained `post_ffn_add` in layer $i$ computes normalized activation directly for layer $i+1$'s input norm (and `final_norm` on the final layer), eliminating 47 redundant `recordRmsNorm` dispatches and barriers per token.
