...
fn runPrompt(
    ...
    const tokens = try tok.encode(allocator, prompt_str, true);
...
pub fn main() !void {
    ...
    // V1: Current tui prompt
    var p1_buf: [8192]u8 = undefined;
    const p1 = try std.fmt.bufPrint(&p1_buf, "<|turn>system\n<|think|>\n{s}\n<turn|>\n<|turn>user\nHow are you doing today?<turn|>\n<|turn>model\n", .{kernel_bytes});
    try runPrompt(allocator, &gpu_ctx, &m, &config, &tok, p1, "1. Current TUI prompt (system with <|think|> and kernel)");

    // V2: Simple system prompt
    const p2 = "<|turn>system\n<|think|>\nYou are a helpful assistant.<turn|>\n<|turn>user\nHow are you doing today?<turn|>\n<|turn>model\n";
    try runPrompt(allocator, &gpu_ctx, &m, &config, &tok, p2, "2. Simple system prompt with <|think|>");

    // V3: Simple system prompt without <|think|>
    const p3 = "<|turn>system\nYou are a helpful assistant.<turn|>\n<|turn>user\nHow are you doing today?<turn|>\n<|turn>model\n";
    try runPrompt(allocator, &gpu_ctx, &m, &config, &tok, p3, "3. Simple system prompt WITHOUT <|think|>");
}
