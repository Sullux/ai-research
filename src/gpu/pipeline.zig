...
pub const ComputePipeline = struct {
    ...
    pub fn init(ctx: *const context.GpuContext, spirv_code: []const u32, num_bindings: u32, push_constant_size: u32) !ComputePipeline {
        // 1. Shader Module
        const sm_info = types_dispatch.VkShaderModuleCreateInfo{
        ...
        const dsl_info = types_dispatch.VkDescriptorSetLayoutCreateInfo{
        ...
        // 3. Pipeline Layout
        const push_range = types_dispatch.VkPushConstantRange{
            .stageFlags = types.VK_SHADER_STAGE_COMPUTE_BIT,
            .offset = 0,
        ...
        const pool_size = types_dispatch.VkDescriptorPoolSize{
            .descriptorType = types.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            .descriptorCount = num_bindings,
        };
        const dp_info = types_dispatch.VkDescriptorPoolCreateInfo{
            .maxSets = 1,
            .poolSizeCount = 1,
        ...
        const ds_alloc = types_dispatch.VkDescriptorSetAllocateInfo{
            .descriptorPool = desc_pool,
            .descriptorSetCount = 1,
            .pSetLayouts = (&desc_set_layout)[0..1].ptr,
        };
        var desc_set: types.VkDescriptorSet = null;
        if (ctx.api.vkAllocateDescriptorSets(ctx.device, &ds_alloc, (&desc_set)[0..1].ptr) != .SUCCESS) return error.VkDescSetAllocFailed;
        ...
        const cb_info = types_dispatch.VkCommandBufferAllocateInfo{
    ...
    pub fn bindBuffers(self: *const ComputePipeline, buffers: []const *const buffer.GpuBuffer) !void {
        var writes = try self.ctx.allocator.alloc(types_dispatch.VkWriteDescriptorSet, buffers.len);
        defer self.ctx.allocator.free(writes);
        var buf_infos = try self.ctx.allocator.alloc(types_dispatch.VkDescriptorBufferInfo, buffers.len);
        defer self.ctx.allocator.free(buf_infos);
        ...
        for (buffers, 0..) |buf, i| {
            ...
            writes[i] = .{
                ...
                .descriptorCount = 1,
                .descriptorType = types.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                .pBufferInfo = (&buf_infos[i])[0..1].ptr,
            };
        }
        self.ctx.api.vkUpdateDescriptorSets(self.ctx.device, @intCast(writes.len), writes.ptr, 0, null);
    }
    ...
    pub fn dispatch(self: *const ComputePipeline, push_constants: ?[]const u8, gx: u32, gy: u32, gz: u32) !void {
        ...
        self.ctx.api.vkCmdBindPipeline(self.cmd_buffer, 1, self.pipeline);
        self.ctx.api.vkCmdBindDescriptorSets(self.cmd_buffer, 1, self.pipeline_layout, 0, 1, (&self.desc_set)[0..1].ptr, 0, null);

        if (push_constants) |pc| {
            self.ctx.api.vkCmdPushConstants(self.cmd_buffer, self.pipeline_layout, types.VK_SHADER_STAGE_COMPUTE_BIT, 0, @intCast(pc.len), pc.ptr);
        }

        self.ctx.api.vkCmdDispatch(self.cmd_buffer, gx, gy, gz);
        _ = self.ctx.api.vkEndCommandBuffer(self.cmd_buffer);

        const submit_info = types_dispatch.VkSubmitInfo{
            .commandBufferCount = 1,
            .pCommandBuffers = (&self.cmd_buffer)[0..1].ptr,
        };
