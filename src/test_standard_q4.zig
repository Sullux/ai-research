...
pub fn quantizeRowQ4_0_Standard(dst_words: []u32, src_bf16: []const u16) void {
...
pub fn main() !void {
    ...
    // Requantize gate and up with llama.cpp signed Q4_0 standard
    for (gpu_model.layers, 0..) |*l_gpu, i| {
        const l_cpu = m.layers[i];
        quantizeRowQ4_0_Standard(l_gpu.gate_proj.asSlice(u32), @as([]const u16, @ptrCast(l_cpu.gate_proj)));
        quantizeRowQ4_0_Standard(l_gpu.up_proj.asSlice(u32), @as([]const u16, @ptrCast(l_cpu.up_proj)));
    }
    ...
    std.debug.print("\n=== Generation with Standard llama.cpp Q4_0 ===\n", .{});
    for (0..120) |_| {
