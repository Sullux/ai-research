...
pub fn main() !void {
    ...
    const kernel_file = try std.fs.cwd().readFile("tui/PROMPT_KERNEL.md", &kernel_buf);
    
    // Replace the system prompt prefix with "You are a helpful and honest assistant."
    const prefix_to_replace = "You are a helpful, respectful and honest AI assistant operating in an AMD Ryzen computing runtime.";
    var combined_buf: [4096]u8 = undefined;
    if (std.mem.startsWith(u8, kernel_file, prefix_to_replace)) {
        const rest = kernel_file[prefix_to_replace.len..];
        const new_kernel = try std.fmt.bufPrint(&combined_buf, "You are a helpful and honest assistant.{s}", .{rest});
        try testPrompt(allocator, &gpu_ctx, &m, &config, &tok, new_kernel, "New Kernel with Standard Assistant Prefix");
    }
}
