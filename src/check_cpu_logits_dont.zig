...
pub fn main() !void {
    ...
    var clock: usize = 0;
    for (tokens) |t| {
        _ = m.forwardToken(&ring, &scratch, t, clock, &pool, null, null, null, false);
        clock += 1;
    }
    ...
    for (gen_prefix, 0..) |t, i| {
        _ = m.forwardToken(&ring, &scratch, t, clock, &pool, null, null, null, i == gen_prefix.len - 1);
        clock += 1;
    }
    ...
    std.debug.print("CPU BF16 Top 10 logits after \"... I don'\":\n", .{});
    for (all[0..10]) |item| {
