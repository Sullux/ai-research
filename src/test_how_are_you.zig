...
pub fn main() !void {
    ...
    for (0..150) |step| {
        ...
        if (step > 60 and step < 80) {
            // print top 3
            var top3: [3]struct { id: u32, val: f32 } = undefined;
            for (&top3) |*t| t.* = .{ .id = 0, .val = -1e9 };
            for (scratch.logits, 0..) |val, id| {
                if (id == 0 or id == 2 or id == 258882 or id == 258883) continue;
                for (0..3) |j| {
                    if (val > top3[j].val) {
                        var k: usize = 2;
                        while (k > j) : (k -= 1) top3[k] = top3[k - 1];
                        top3[j] = .{ .id = @intCast(id), .val = val };
                        break;
                    }
                }
            }
            std.debug.print("\n[clk {d:4}] next token candidates:\n", .{clock});
            for (top3) |t| std.debug.print("    token {d:6} ('{s}'): {d:.3}\n", .{ t.id, tok.decode(t.id), t.val });
        }
    }
    std.debug.print("\nEnd at clock {}\n", .{clock});
}
