...
pub fn main() !void {
    ...
    const test_lens = [_]usize{ 10, 50, 100, 200, 390 };
    for (test_lens) |test_len| {
        ...
        const sub_len = @min(test_len, all_tokens.len);
        const tokens = all_tokens[0..sub_len];
        ...
        for (batch_logits) |v| if (v > max_logit) { max_logit = v; };
        std.debug.print("Tokens {d:3}: top token {} ('{s}'), max_logit = {d:.3}\n", .{ sub_len, top_tok, tok.decode(top_tok), max_logit });
    }
}
