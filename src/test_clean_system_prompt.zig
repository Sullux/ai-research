...
pub fn main() !void {
    ...
    // Extract tools part from kernel_file
    const tool_idx = std.mem.indexOf(u8, kernel_file, "<|tool>") orelse 0;
    const tools_part = kernel_file[tool_idx..];

    var combined_buf: [4096]u8 = undefined;
    const clean_kernel = try std.fmt.bufPrint(&combined_buf, "You are a helpful assistant with access to tools.{s}", .{tools_part});
    ...
    std.debug.print("\n=== Generation with Clean System Prompt Kernel in Q4 Mode ===\n", .{});
    for (0..120) |_| {
