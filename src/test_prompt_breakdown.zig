...
pub fn main() !void {
    ...
    // Test with full tui/PROMPT_KERNEL.md
    try testPrompt(allocator, &gpu_ctx, &m, &config, &tok, kernel_file, "Full PROMPT_KERNEL.md");

    // Test with newline before <|tool>
    var mod_buf: [4096]u8 = undefined;
    const tool_pos = std.mem.indexOf(u8, kernel_file, "<|tool>") orelse 0;
    const mod_kernel = try std.fmt.bufPrint(&mod_buf, "{s}\n{s}", .{ kernel_file[0..tool_pos], kernel_file[tool_pos..] });
    try testPrompt(allocator, &gpu_ctx, &m, &config, &tok, mod_kernel, "PROMPT_KERNEL with newline before tools");
}
