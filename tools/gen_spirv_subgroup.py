...
def generate_subgroup_q4():
    """
    High-Occupancy Wave-Cooperative Q4_0 GEMV with Subgroup Reduction:
    Workgroup: (256, 1, 1) -> 8 Wave32 wavefronts per workgroup.
    Each workgroup computes 8 rows: row = gl_WorkGroupID.x * 8 + wave_id.
    wave_id = gl_LocalInvocationID.x >> 5 (0..7)
    lane = gl_LocalInvocationID.x & 31 (0..31)
    
    Workgroup shared memory:
      sx_data[1024]: 1024 floats loaded ONCE cooperatively by all 256 threads (4 floats/thread).
    
    Each wave w computes row r using coalesced 5-word loads from W.
    Reduction within each wave uses OpGroupNonUniformFAdd (Scope Subgroup = 3).
    Lane 0 of each wave writes Y[row].
    """
    instructions = []
    def emit(op, *args):
        words = []
    ...
    c_uint_subgroup_scope = get_id() # 3 = Subgroup
    c_uint_barrier_scope = get_id() # 2 = Workgroup
    c_uint_barrier_semantics = get_id() # 0x108 = WorkgroupMemory | AcquireRelease
    
    main_func = get_id()
    
    emit(17, 1) # OpCapability Shader
    emit(17, 61) # OpCapability GroupNonUniform
    emit(17, 63) # OpCapability GroupNonUniformArithmetic
    emit(14, 0, 1) # OpMemoryModel Logical GLSL450
    emit(15, 5, main_func, "main", var_gid, var_lid)
    emit(16, main_func, 1, 256, 1, 1) # LocalSize 256 1 1
    ...
    emit(43, uint_t, c_uint_31, 31)
    emit(43, float_t, c_float_0, 0.0)
    emit(43, float_t, c_float_8, 8.0)
    emit(43, uint_t, c_uint_subgroup_scope, 3) # Scope Subgroup = 3
    ...
    gid_x = get_id()
    emit(61, uint_t, gid_x, gid_x_ptr)
    ...
    wave_id = get_id()
    emit(138, uint_t, wave_id, tid, c_uint_5_shift)
    
    # lane = tid & 31
    lane = get_id()
    emit(134, uint_t, lane, tid, c_uint_31)
    
    # row = gid_x * 8 + wave_id = (gid_x << 3) + wave_id
    c_uint_3_shift = get_id()
    emit(43, uint_t, c_uint_3_shift, 3)
    gid_x8 = get_id()
    emit(137, uint_t, gid_x8, gid_x, c_uint_3_shift)
    row = get_id()
    emit(128, uint_t, row, gid_x8, wave_id)
    ...
    # Outer loop: chunk = 0; chunk < num_blocks; chunk += 32
    ...
    # All 256 threads cooperatively load 1024 floats of X (4 floats per thread):
    # sx_data[tid], sx_data[tid+256], sx_data[tid+512], sx_data[tid+768]
    chunk_k_base = get_id()
    emit(137, uint_t, chunk_k_base, phi_chunk, c_uint_5_shift)
    
    for step in range(4):
        c_step_offset = get_id()
        emit(43, uint_t, c_step_offset, step * 256)
        sx_idx = get_id()
        emit(128, uint_t, sx_idx, tid, c_step_offset)
        
        glob_x_idx = get_id()
        emit(128, uint_t, glob_x_idx, chunk_k_base, sx_idx)
        
        cond_x = get_id()
        emit(170, bool_t, cond_x, glob_x_idx, val_k)
        lbl_load_x = get_id()
        lbl_zero_x = get_id()
        lbl_merge_x = get_id()
        emit(245, cond_x, lbl_load_x, lbl_zero_x)
        ...
        emit(249, lbl_merge_x)
        
        emit(248, lbl_zero_x)
        emit(249, lbl_merge_x)
        
        emit(248, lbl_merge_x)
        phi_x = get_id()
        emit(244, float_t, phi_x, gx_val, lbl_load_x, c_float_0, lbl_zero_x)
        
        sx_ptr = get_id()
        emit(65, ptr_workgroup_float_t, sx_ptr, var_sx, sx_idx)
        emit(62, sx_ptr, phi_x)
        
    emit(224, c_uint_barrier_scope, c_uint_barrier_scope, c_uint_barrier_semantics)
    
    # Check if this wave's row < M
    cond_valid_row = get_id()
    emit(170, bool_t, cond_valid_row, row, val_m)
    
    lbl_compute_wave = get_id()
    lbl_compute_wave_merge = get_id()
    emit(245, cond_valid_row, lbl_compute_wave, lbl_compute_wave_merge)
    
    emit(248, lbl_compute_wave)
    # blk_global = phi_chunk + lane
    blk_global = get_id()
    emit(128, uint_t, blk_global, phi_chunk, lane)
    cond_valid_blk = get_id()
    emit(170, bool_t, cond_valid_blk, blk_global, num_blocks)
    
    lbl_compute_blk = get_id()
    lbl_compute_blk_merge = get_id()
    emit(245, cond_valid_blk, lbl_compute_blk, lbl_compute_blk_merge)
    
    emit(248, lbl_compute_blk)
    # blk_w_offset = row_base + blk_global * 5
    blk_x5 = get_id()
    emit(132, uint_t, blk_x5, blk_global, c_uint_5)
    blk_w_offset = get_id()
    emit(128, uint_t, blk_w_offset, row_base, blk_x5)
    
    # Load d
    d_ptr = get_id()
    emit(65, ptr_sb_uint_t, d_ptr, var_w, c_uint_0, blk_w_offset)
    d_u32 = get_id()
    ...
    for word_idx in range(4):
        ...
        emit(128, uint_t, w_offset, blk_w_offset, c_w_idx)
        ...
        for nib_idx in range(8):
            ...
            # Read from shared memory sx_data
            sx_read_ptr = get_id()
    ...
    emit(249, lbl_compute_blk_merge)
    
    emit(248, lbl_compute_blk_merge)
    phi_blk_val = get_id()
    emit(244, float_t, phi_blk_val, blk_sum, lbl_compute_blk, c_float_0, lbl_compute_wave)
    emit(249, lbl_compute_wave_merge)
    
    emit(248, lbl_compute_wave_merge)
    phi_wave_val = get_id()
    emit(244, float_t, phi_wave_val, phi_blk_val, lbl_compute_blk_merge, c_float_0, label_loop_body)
    
    new_phi_sum = get_id()
    emit(129, float_t, new_phi_sum, phi_sum, phi_wave_val)
    ...
    # 1-cycle Subgroup Reduction across the 32 lanes of the wave:
    # OpGroupNonUniformFAdd: Opcode 334 (Scope Subgroup = 3, Operation Reduce = 0)
    subgroup_sum = get_id()
    emit(334, float_t, subgroup_sum, c_uint_subgroup_scope, c_uint_0, phi_sum)
    
    # If lane == 0 and row < M: Y[row] = subgroup_sum
    cond_lane0 = get_id()
    emit(170, bool_t, cond_lane0, lane, c_uint_1)
    
    cond_write = get_id()
    emit(164, bool_t, cond_write, cond_lane0, cond_valid_row) # OpLogicalAnd
...
if __name__ == '__main__':
    ...
    with open("src/gpu/shaders_q4.zig", "w") as f:
        ...
        if len(q4_code) % 16 != 0:
            f.write('\n')
        f.write('};\n')
    print(f"Generated and wrote subgroup Q4 SPIR-V: {len(q4_code)} words ({len(q4_code)*4} bytes)")
