...
def generate_branchless_subgroup_q4():
    """
    Branchless High-Throughput Wave-Cooperative Q4_0 GEMV with Subgroup Reduction:
    Workgroup: (256, 1, 1) -> 8 Wave32 wavefronts per workgroup.
    Computes 8 rows: row = gl_WorkGroupID.x * 8 + wave_id.
    Inner loop has ZERO branches/divergence.
    100% straight-line SIMD instructions.
    """
    instructions = []
    def emit(op, *args):
        words = []
    ...
    # if row >= M, clamp row to 0 to prevent OOB read, and we won't write Y at the end
    cond_valid_row = get_id()
    emit(170, bool_t, cond_valid_row, row, val_m)
    
    safe_row = get_id()
    emit(169, uint_t, safe_row, cond_valid_row, row, c_uint_0) # OpSelect
    ...
    # Loop over chunks: chunk = 0; chunk < num_blocks; chunk += 32
    ...
    # 256 threads load 1024 floats of X into LDS
    chunk_k_base = get_id()
    ...
    # Straight-line block compute (zero branches)
    blk_global = get_id()
...
if __name__ == '__main__':
    q4_code = generate_branchless_subgroup_q4()
    with open("src/gpu/shaders_q4.zig", "w") as f:
        ...
        if len(q4_code) % 16 != 0:
            f.write('\n')
        f.write('};\n')
    print(f"Generated and wrote branchless subgroup Q4 SPIR-V: {len(q4_code)} words ({len(q4_code)*4} bytes)")
